...
from app.api.api_v1.common.consts import JWT_SECRET, JWT_ALGORITHM
from app.api.api_v1.database.conn import db 
from app.api.api_v1.database.schema import Users
...
9. 각 SNS 에서 Unlink
10. 회원 탈퇴
11. 탈퇴 회원정보 저장 시간 동안 보유(법적 최대 한도차 내에서, 가입 때 약관 동의 받아야 함, 재가입 방지 용도로 사용하면 가능)
...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        # hash_pw = bcrypt.hashpw(reg_info.pw.encode("utf-8"), bcrypt.gensalt()) 임시
        hash_pw = reg_info.pw.encode("utf-8") # 임시로 암호화해제
...
async def login(sns_type: SnsType, user_info: UserRegister):
    if sns_type == SnsType.email:
        ...
        # 유저 패스워드 확인
        # is_verified = bcrypt.checkpw(user_info.pw.encode("utf-8"), user.pw.encode("utf-8")) 임시
        is_verified = False # 임시로 암호화해제
        if user_info.pw.encode("utf-8") == user.pw.encode("utf-8"): # 임시로 암호화해제
            is_verified = True # 임시로 암호화해제
